...
int airplay2_info(rtsp_instance_t *s, rtsp_client_t *c, const rtsp_request_t *r)
{
    ...
    add_int(&w, refs, &n, "vv", 2);
    add_int(&w, refs, &n, "statusFlags", 4);
    add_int(
        &w, refs, &n, "features", (uint64_t)AIRPLAY2_FEATURES_HIGH << 32 | AIRPLAY2_FEATURES_LOW);
    uint32_t formats[2];
...
static int stream_key(const bplist_t *p, uint32_t stream, pairing_t *pair, uint8_t key[32])
{
    ...
    if (!result)
        result = mbedtls_chachapoly_auth_decrypt(&ctx, 32, nonce, NULL, 0, data + 32, data, key);
...
static int setup(rtsp_instance_t *s, rtsp_client_t *c, const rtsp_request_t *r, const bplist_t *p)
{
    ...
    session.sample_rate = (uint32_t)rate;
    session.channels = 2;
    session.bits_per_sample = 16;
    session.frames_per_packet = (uint32_t)frames;
    ...
    s->stream.timing_peer = c->peer;
    s->stream.timing_peer.port = 0;
                      ...
                      const bplist_t *p)
{
    ...
    {
        os_mutex_unlock(&s->state_lock);
        return status(c, r, 455, "Invalid State");
    }
    if (a.playing)
        s->stream.anchor = a;
    else
