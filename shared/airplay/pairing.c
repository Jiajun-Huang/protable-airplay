...
#define TRY(call) do { if ((call) != 0) goto done; } while (0)
...
static void free_srp(pairing_t *p)
{
    srp_t *s = p->srp;
    if (!s) return;
    mbedtls_mpi_free(&s->n); mbedtls_mpi_free(&s->v); mbedtls_mpi_free(&s->b);
    mbedtls_platform_zeroize(s, sizeof(*s)); free(s); p->srp = NULL;
}
void pairing_close(pairing_t *p)
{
    if (!p) return;
    free_srp(p); mbedtls_platform_zeroize(p, sizeof(*p));
}

/* Concatenate TLV8 fragments, rejecting truncation and duplicate scalar fields. */
static int tlv_get(const uint8_t *in, size_t size, uint8_t type,
                   uint8_t *out, size_t capacity, size_t *length)
{
    ...
    while (pos < size) {
        ...
        if (n > size - pos) return -1;
        if (t == type) {
            if ((found && (previous != type || previous_size != 255)) || n > capacity - used)
                return -1;
            if (n) memcpy(out + used, in + pos, n);
            used += n; found = 1;
        }
        previous = (int)t; previous_size = n; pos += n;
    }
    *length = used;
    return found ? 0 : -1;
}
static int tlv_put(uint8_t *out, size_t capacity, size_t *used,
                   uint8_t type, const uint8_t *data, size_t size)
{
    do {
        size_t n = size > 255 ? 255 : size;
        if (*used > capacity || n + 2 > capacity - *used) return -1;
        out[(*used)++] = type; out[(*used)++] = (uint8_t)n;
        if (n) memcpy(out + *used, data, n);
        *used += n; data += n; size -= n;
    } while (size);
    return 0;
}
static int hash_parts(uint8_t out[64], const uint8_t *const *parts, const size_t *sizes, size_t count)
{
    ...
    if (!result) result = mbedtls_sha512_finish_ret(&h, out);
...
static int begin(pairing_t *p)
{
    ...
    mbedtls_mpi_init(&g); mbedtls_mpi_init(&x); mbedtls_mpi_init(&k);
    mbedtls_mpi_init(&temp); mbedtls_mpi_init(&pub);
    ...
    if (!s) goto done;
    p->srp = s;
    mbedtls_mpi_init(&s->n); mbedtls_mpi_init(&s->v); mbedtls_mpi_init(&s->b);
    TRY(mbedtls_ctr_drbg_seed(&rng, mbedtls_entropy_func, &entropy,
                              (const uint8_t *)"AirPlay-Pair-Setup", 18));
    ...
    mbedtls_mpi_free(&g); mbedtls_mpi_free(&x); mbedtls_mpi_free(&k);
    mbedtls_mpi_free(&temp); mbedtls_mpi_free(&pub);
    mbedtls_ctr_drbg_free(&rng); mbedtls_entropy_free(&entropy);
    if (result) free_srp(p);
    return result;
}
...
static int verify(pairing_t *p, const uint8_t *a, size_t a_size,
                   const uint8_t proof[64], uint8_t answer[64])
{
    srp_t *s = p->srp;
    if (!s || !a_size || a_size > SRP_BYTES) return -1;
    int result = -1;
    mbedtls_mpi A, u, temp, secret;
    mbedtls_mpi_init(&A); mbedtls_mpi_init(&u); mbedtls_mpi_init(&temp); mbedtls_mpi_init(&secret);
    ...
    if (mbedtls_mpi_cmp_int(&temp, 0) == 0) goto done;
    ...
    if (mbedtls_mpi_cmp_int(&u, 0) == 0) goto done;
    ...
    if (!secret_size || secret_size > sizeof(secret_bytes)) goto done;
    ...
    for (size_t i = 2; i <= 4; ++i) minimal(&parts[i], &sizes[i]);
    TRY(hash_parts(expected, parts, sizes, 6));
    unsigned difference = 0;
    for (size_t i = 0; i < 64; ++i) difference |= expected[i] ^ proof[i];
    if (difference) goto done;
    ...
    TRY(mbedtls_hkdf(md, (const uint8_t *)"Control-Salt", 12, key, 64,
                     (const uint8_t *)"Control-Read-Encryption-Key", 27, p->write_key, 32));
    TRY(mbedtls_hkdf(md, (const uint8_t *)"Control-Salt", 12, key, 64,
                     (const uint8_t *)"Control-Write-Encryption-Key", 28, p->read_key, 32));
    memcpy(p->shared_secret, key, 32);
    p->established = 1;
    result = 0;
done:
    mbedtls_platform_zeroize(key, sizeof(key)); mbedtls_platform_zeroize(secret_bytes, sizeof(secret_bytes));
    mbedtls_platform_zeroize(hash, sizeof(hash)); mbedtls_platform_zeroize(expected, sizeof(expected));
    mbedtls_mpi_free(&A); mbedtls_mpi_free(&u); mbedtls_mpi_free(&temp); mbedtls_mpi_free(&secret);
    return result;
}

int pairing_setup(pairing_t *p, const uint8_t *in, size_t size,
                  uint8_t *out, size_t capacity, size_t *out_size)
{
    uint8_t state, flags = 0, method = 0;
    size_t n, used = 0;
    if (!p || !in || !out || !out_size || capacity < 6) return -1;
    *out_size = 0;
    if (tlv_get(in, size, 6, &state, 1, &n) || n != 1 || p->established) return -1;
    int ok = 0;
    if (state == 1) {
        if (!tlv_get(in, size, 0, &method, 1, &n) && n == 1 && method == 0 &&
            !tlv_get(in, size, 0x13, &flags, 1, &n) && n == 1 && (flags & 0x10) &&
            !begin(p)) {
            srp_t *s = p->srp;
            uint8_t next = 2;
    ...
    if (!ok) {
        ...
        if (tlv_put(out, capacity, &used, 6, &next, 1) ||
            tlv_put(out, capacity, &used, 7, &error, 1)) return -1;
    }
    *out_size = used;
    return 0;
}
...
{
    if (!p || !p->established || !in || !size || size > PAIR_RECORD_MAX ||
        !record || p->write_counter == UINT64_MAX) return -1;
    uint8_t nonce[12]; nonce_bytes(nonce, p->write_counter);
    record[0] = (uint8_t)size; record[1] = (uint8_t)(size >> 8);
    mbedtls_chachapoly_context c; mbedtls_chachapoly_init(&c);
    int result = mbedtls_chachapoly_setkey(&c, p->write_key);
    if (!result) result = mbedtls_chachapoly_encrypt_and_tag(&c, size, nonce, record, 2,
                                                           in, record + 2, record + 2 + size);
...
int pairing_open(pairing_t *p, const uint8_t *record, size_t size, uint8_t *out)
{
    ...
    if (!n || n > PAIR_RECORD_MAX || size != n + 18) return -1;
    uint8_t nonce[12]; nonce_bytes(nonce, p->read_counter);
    mbedtls_chachapoly_context c; mbedtls_chachapoly_init(&c);
    int result = mbedtls_chachapoly_setkey(&c, p->read_key);
    if (!result) result = mbedtls_chachapoly_auth_decrypt(&c, n, nonce, record, 2,
                                                       record + 2 + n, record + 2, out);
    mbedtls_chachapoly_free(&c);
    if (result) return -1;
    ++p->read_counter;
    return (int)n;
