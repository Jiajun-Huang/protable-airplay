...
{
    if (!stage || !request || !response || size < 12 || memcmp(request, "FPLY", 4) ||
        request[4] != 3 || request[5] != 1 || request[7] != 0)
        return -1;
    uint32_t payload_size = (uint32_t)request[8] << 24 | (uint32_t)request[9] << 16 |
