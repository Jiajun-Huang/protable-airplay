#include "fairplay.h"
#include <string.h>

/* Fixed version 3 wire responses, indexed by the request mode (0..3).
 * Protocol data cross-checked against rbouteiller/airplay-esp32,
 * main/rtsp/rtsp_fairplay.c at 9ef96118e5ed149958c7913d9708824f5b97cb13. */
static const uint8_t setup_responses[4][FAIRPLAY_RESPONSE_MAX] = {
    {
    },
    {
    },
    {
    },
    {
    }
};

int fairplay_setup(uint8_t *stage, const uint8_t *request, size_t size,
                  uint8_t *response, size_t capacity)
{
    if (!stage || !request || !response || size < 12 ||
        memcmp(request, "FPLY", 4) || request[4] != 3 || request[5] != 1 ||
        request[7] != 0)
        return -1;
    uint32_t payload_size = (uint32_t)request[8] << 24 |
                            (uint32_t)request[9] << 16 |
                            (uint32_t)request[10] << 8 | request[11];
    if (payload_size != size - 12)
        return -1;
    if (request[6] == 1) {
        if (size != 16 || request[14] > 3 || capacity < FAIRPLAY_RESPONSE_MAX)
            return -1;
        memcpy(response, setup_responses[request[14]], FAIRPLAY_RESPONSE_MAX);
        *stage = 1;
        return FAIRPLAY_RESPONSE_MAX;
    }
    if (request[6] == 3) {
        if (*stage != 1 || size != 164 || capacity < 32)
            return -1;
        memcpy(response, "FPLY\x03\x01\x04\x00\x00\x00\x00\x14", 12);
        memcpy(response + 12, request + size - 20, 20);
        *stage = 2;
        return 32;
    }
    return -1;
}
