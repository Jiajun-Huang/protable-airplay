...
/**
 * @brief audio_pipeline_decode_alac.
 * @param pipeline Parameter named pipeline.
 * @param payload Parameter named payload.
...
/**
 * @brief audio_pipeline_decode_pcm.
 * @param pipeline Parameter named pipeline.
 * @param payload Parameter named payload.
 * @param payload_len Parameter named payload_len.
 * @param output Parameter named output.
...
/**
 * @brief audio_pipeline_read_bits.
 * @param data Parameter named data.
 * @param bit_length Parameter named bit_length.
 * @param bit_offset Parameter named bit_offset.
...
/**
 * @brief audio_pipeline_decode_aac.
 * @param pipeline Parameter named pipeline.
 * @param payload Parameter named payload.
 * @param payload_len Parameter named payload_len.
 * @param output Parameter named output.
...
/**
 * @brief audio_pipeline_decode_packet.
...
static void audio_pipeline_decode_packet(const rtp_packet_t *packet, void *user_data)
{
    ...
    if (!pipeline)
    {
        ...
        if (!logged_null++)
            LOG_ERROR("RTP audio callback received NULL pipeline!\n");
        return;
    }
    ...
    if (pipeline->state != AUDIO_PIPELINE_PLAYING)
    {
        ...
        if (!logged_state++)
            LOG_WARN("RTP audio callback but pipeline not PLAYING (state=%d)\n", pipeline->state);
        return;
    }
    ...
    if (pipeline->session.codec == SDP_CODEC_ALAC)
    {
            ...
            0)
        {
            ...
            if (pipeline->decode_errors <= 3 || pipeline->decode_errors % 100 == 0)
                LOG_ERROR("ALAC rejected: seq=%u bytes=%zu encrypted=%d errors=%u\n",
    ...
    else if (pipeline->session.codec == SDP_CODEC_AAC)
    {
            ...
            0)
        {
            ++pipeline->decode_errors;
            if (pipeline->decode_errors <= 3 || pipeline->decode_errors % 100 == 0)
                LOG_ERROR("AAC rejected: seq=%u bytes=%zu errors=%u\n",
                          packet->header.sequence,
                          packet->payload_len,
                          pipeline->decode_errors);
            return;
    ...
    {
        LOG_ERROR("Unsupported codec: seq=%u payload_type=%u codec=%d\n",
                  packet->header.sequence,
                  packet->header.payload_type,
                  pipeline->session.codec);
        return;
    }
    ...
    if (decoded_count > 0 && pipeline->on_audio_data)
    {
        ...
        {
            LOG_DEBUG("First decoded packet: seq=%u pt=%u bytes=%zu samples=%zu channels=%u "
                      "peak=%d encrypted=%d\n",
                      packet->header.sequence,
                      packet->header.payload_type,
                      packet->payload_len,
    ...
    {
        LOG_DEBUG("Played %u packets, gaps %u (%.2f%%), nonzero=%u queued=%u late=%u overflow=%u\n",
                  pipeline->packets_received,
...
/**
 * @brief audio_pipeline_on_rtp_audio.
 * @param packet Parameter named packet.
 * @param user_data Parameter named user_data.
 */
static void audio_pipeline_on_rtp_audio(const rtp_packet_t *packet, void *user_data)
{
    ...
    if (pipeline->session.stream_type)
    {
                                        ...
                                        &decoded.payload_len))
        {
            if (++pipeline->decode_errors <= 3)
                LOG_ERROR("Audio authentication failed: seq=%u\n", packet->header.sequence);
            return;
        }
    ...
    if (result < 0 && ++pipeline->queue_overflows <= 3)
        LOG_WARN("Playout capacity exceeded: queued=%u bytes=%zu\n",
                 pipeline->playout.count,
                 packet->payload_len);
    if (result > 0 && !pipeline->first_arrival_us)
    {
        ...
        LOG_DEBUG("Buffering first RTP packet: seq=%u timestamp=%u bytes=%zu\n",
                  packet->header.sequence,
...
/**
 * @brief audio_pipeline_on_rtp_control.
 * @param data Parameter named data.
...
static void audio_pipeline_on_rtp_control(const uint8_t *data, size_t len, void *user_data)
{
    ...
    if (pipeline->session.stream_type)
    {
        ...
        if (pipeline->session.stream_type == AIRPLAY_STREAM_TYPE_REALTIME &&
            !ptp_sync_anchor(&anchor, data, len))
        {
            if (!pipeline->anchor.valid || pipeline->anchor.clock_id != anchor.clock_id)
                LOG_DEBUG("Realtime anchor RTP=%u clock=%016llx\n",
                          anchor.rtp_time,
    ...
    if (ntp_sync_control(&pipeline->ntp_sync, data, len, rate) == 0 &&
        ...
        LOG_DEBUG("RTP anchor=%u sender latency=%u frames (%.1f ms), clock_ready=%d\n",
...
/**
 * @brief audio_pipeline_on_rtp_timing.
 * @param data Parameter named data.
 * @param len Parameter named len.
                                         ...
                                         void *user_data)
{
    ...
    if (ntp_sync_process_packet(&pipeline->ntp_sync, data, len) == 0 && !had_clock)
        LOG_DEBUG("Sender clock ready: RTT=%lld us offset=%lld us\n",
...
int audio_pipeline_create(audio_pipeline_t *pipeline, const audio_pipeline_config_t *config)
{
    ...
    {
        LOG_WARN("Failed to create NTP sync\n");
    }
    ...
    {
        LOG_ERROR("Failed to create RTP receiver\n");
    ...
    LOG_INFO("Created audio pipeline (ports: %u/%u/%u)\n",
             config->audio_port,
             config->control_port,
             config->timing_port);
...
int audio_pipeline_configure(audio_pipeline_t *pipeline, const sdp_session_t *session)
{
    ...
    LOG_INFO("Configured: codec=%d, %uHz, %u-ch, %u-bit, %u frames/pkt\n",
             session->codec,
             session->sample_rate,
             session->channels,
    ...
    if (session->codec == SDP_CODEC_UNKNOWN)
    {
        ...
        LOG_WARN("Unknown codec in SDP; falling back to ALAC\n");
    }
    ...
    if (impl->session.codec == SDP_CODEC_ALAC && impl->alac_decoder.frame_length == 0)
    {
        ...
        if (alac_decoder_init(&impl->alac_decoder,
                              ...
                              session->sample_rate) != 0)
        {
            LOG_ERROR("Failed to initialize ALAC decoder\n");
            return -1;
        }
    }
    ...
    if (session->has_encryption)
    {
        ...
        if (session->aes_key_encrypted_len > 0)
        {
            ...
            {
                have_key = 1;
                LOG_INFO("AES key decrypted via RSA\n");
            }
            else
            {
                LOG_ERROR("RSA AES-key decrypt failed\n");
            }
        }
        ...
        {
            LOG_INFO("AES-128-CBC decryption enabled\n");
        }
        else
        {
...
int audio_pipeline_start(audio_pipeline_t *pipeline)
{
    ...
    if (!impl->configured)
    {
        LOG_ERROR("Cannot start - not configured\n");
        return -1;
    }
...
static int audio_pipeline_flush(audio_pipeline_t *pipeline)
{
    ...
    LOG_DEBUG("Flushed audio buffers\n");
...
int audio_pipeline_stop(audio_pipeline_t *pipeline)
{
    ...
    LOG_INFO("Stopped playback\n");

    return 0;
}

/* A deadline resolver returns 0 when ready; otherwise the packet stays queued. */
typedef int (*pipeline_deadline_fn)(audio_pipeline_t *pipeline,
                                    uint32_t timestamp,
                                    uint64_t now_us,
...
static int pipeline_poll_inputs(audio_pipeline_t *pipeline, int timeout_ms)
{
    ...
    if (pipeline->state == AUDIO_PIPELINE_PLAYING && (timeout_ms < 0 || timeout_ms > 2))
        timeout_ms = 2;
    /* Keep draining open UDP sockets while idle so old packets cannot accumulate. */
    if (rtp_receiver_poll(&pipeline->rtp, timeout_ms) != 0)
        return -1;
    if (ptp_sync_poll(&pipeline->ptp))
        return -1;
    return 0;
}

static int pipeline_receive_buffered(audio_pipeline_t *pipeline)
{
    if (pipeline->state != AUDIO_PIPELINE_PLAYING)
        return 0;

    /* Stop reading before the compressed queue fills; TCP supplies back-pressure. */
    unsigned limit = pipeline->buffered.discarding ? 256 : 32;
    for (unsigned i = 0; i < limit && (pipeline->buffered.discarding ||
                                       pipeline->playout.count < AIRPLAY_PLAYOUT_PACKETS - 2);
         ++i)
    {
        int result = buffered_audio_poll(
            &pipeline->buffered, pipeline->timing_peer.ip, audio_pipeline_on_rtp_audio, pipeline);
        if (result < 0)
            return -1;
        if (!result)
            break;
...
static int pipeline_ntp_deadline(audio_pipeline_t *pipeline,
                                 uint32_t timestamp,
                                 uint64_t now_us,
                                 uint64_t *deadline_us)
{
    if (ntp_sync_deadline(
            &pipeline->ntp_sync, timestamp, pipeline->session.sample_rate, deadline_us) == 0)
        return 0;

    if (pipeline->timing_peer.port && now_us - pipeline->first_arrival_us < 3000000)
        return -1;
    if (!pipeline->fallback_logged)
    {
        LOG_WARN("No sender clock/anchor; using relative 2 s buffering\n");
        pipeline->fallback_logged = 1;
    }
    /* The fallback timeline remains anchored to arrival, including after the sync wait. */
    *deadline_us = pipeline->first_arrival_us + 2000000 +
                   (int64_t)(int32_t)(timestamp - pipeline->first_timestamp) * 1000000 /
                       pipeline->session.sample_rate;
    return 0;
}

static int pipeline_ptp_deadline(audio_pipeline_t *pipeline,
                                 uint32_t timestamp,
                                 uint64_t now_us,
                                 uint64_t *deadline_us)
{
    (void)now_us;
    return ptp_sync_deadline(&pipeline->ptp,
                             &pipeline->anchor,
                             timestamp,
                             pipeline->session.sample_rate,
                             pipeline->session.latency_frames,
                             deadline_us);
}

static int pipeline_play_ready_packets(audio_pipeline_t *pipeline,
                                       pipeline_deadline_fn get_deadline)
{
    for (unsigned batch = 0; batch < 16 && pipeline->state == AUDIO_PIPELINE_PLAYING; ++batch)
    {
        playout_packet_t *queued = playout_peek(&pipeline->playout);
        if (!queued)
            break;
        uint64_t now_us = os_time_us();
        uint64_t deadline_us;
        if (get_deadline(pipeline, queued->header.timestamp, now_us, &deadline_us) != 0)
            break; //

        /* Bound native output buffering even when the sender transmits seconds ahead. */
        if ((int64_t)(deadline_us - now_us) > 40000)
            break;
        int output_delay_frames =
            pipeline->output_delay_frames ? pipeline->output_delay_frames(pipeline->user_data) : 0;
        if (output_delay_frames < 0)
            return -1;
        int64_t output_delay_us =
            (int64_t)output_delay_frames * 1000000 / pipeline->session.sample_rate;
        int64_t submit_margin_us = (int64_t)(deadline_us - now_us) - output_delay_us;
        if (submit_margin_us > 5000)
            break;
        if (submit_margin_us < -100000)
        {
            ++pipeline->late_packets;
        }
        else
        {
            ...
            audio_pipeline_decode_packet(&packet, pipeline);
        }
        playout_pop(&pipeline->playout, queued);
    }
    return 0;
}

static int pipeline_poll_raop(audio_pipeline_t *pipeline, int timeout_ms)
{
    if (pipeline_poll_inputs(pipeline, timeout_ms) != 0)
        return -1;
    return pipeline_play_ready_packets(pipeline, pipeline_ntp_deadline);
}

static int pipeline_poll_airplay2_realtime(audio_pipeline_t *pipeline, int timeout_ms)
{
    if (pipeline_poll_inputs(pipeline, timeout_ms) != 0)
        return -1;
    return pipeline_play_ready_packets(pipeline, pipeline_ptp_deadline);
}

static int pipeline_poll_airplay2_buffered(audio_pipeline_t *pipeline, int timeout_ms)
{
    if (pipeline_poll_inputs(pipeline, timeout_ms) != 0)
        return -1;
    if (pipeline_receive_buffered(pipeline) != 0)
        return -1;
    return pipeline_play_ready_packets(pipeline, pipeline_ptp_deadline);
}
...
int audio_pipeline_poll(audio_pipeline_t *pipeline, int timeout_ms)
{
    ...
    switch (pipeline->session.stream_type)
    {
    case 0:
        return pipeline_poll_raop(pipeline, timeout_ms);
    case 96:
        return pipeline_poll_airplay2_realtime(pipeline, timeout_ms);
    case 103:
        return pipeline_poll_airplay2_buffered(pipeline, timeout_ms);
    default:
        LOG_ERROR("Unknown stream type %d\n", pipeline->session.stream_type);
        return -1;
    }
}
...
void audio_pipeline_close(audio_pipeline_t *pipeline)
{
    ...
    LOG_INFO("Closed audio pipeline\n");
