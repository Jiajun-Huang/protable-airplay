...
{
    net_close(&p->sockets[0]); net_close(&p->sockets[1]);
    ptp_sync_init(p);
}
int ptp_sync_open(ptp_sync_t *p, const char *ip)
{
...
int ptp_sync_packet(ptp_sync_t *p, const uint8_t *data, size_t size, uint64_t now)
{
    ...
    uint64_t sec = be(data + 34, 6), ns = be(data + 40, 4);
    if (ns >= 1000000000 || sec > INT64_MAX / 1000000) return -1;
    int64_t offset = (int64_t)(sec * 1000000 + ns / 1000) + correction - (int64_t)p->sync_received_us;
    ...
    if (!p->ready) LOG_INFO("ptp", "Sender clock ready: %016llx\n", (unsigned long long)clock);
    p->ready = 1; p->pending = 0; p->updated_us = now;
    return 1;
}
int ptp_sync_poll(ptp_sync_t *p)
{
    if (!p->opened) return 0;
    uint8_t data[512]; net_addr_t peer;
    for (unsigned i = 0; i < 2; ++i)
        for (unsigned batch = 0; batch < 8; ++batch) {
            int n = net_udp_recv(&p->sockets[i], data, sizeof(data), &peer, 0);
            if (n == NET_TIMEOUT) break;
            if (n == NET_ERROR) return -1;
...
{
    if (!anchor || !data || size < 28 || (data[0] >> 6) != 2 ||
        (data[1] & 0x7f) != 0x57 || !be(data + 20, 8))
        return -1;
    *anchor = (airplay_anchor_t){.rtp_time = (uint32_t)be(data + 4, 4),
        .network_us = be(data + 8, 8) / 1000, .clock_id = be(data + 20, 8),
        .valid = 1, .playing = 1};
    return 0;
}

int ptp_sync_deadline(const ptp_sync_t *p, const airplay_anchor_t *a,
                      uint32_t timestamp, uint32_t rate, uint32_t latency, uint64_t *deadline)
{
    ...
    if (!p->ready || !a->valid || !a->playing || !rate || a->clock_id != p->clock_id ||
        (now >= p->updated_us && now - p->updated_us > 5000000)) return -1;
