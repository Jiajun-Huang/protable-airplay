...
int bplist_open(bplist_t *p, const uint8_t *data, size_t size)
{
    ...
    if (!tail[6] || tail[6] > 8 || !tail[7] || tail[7] > 8 || !count || count > UINT32_MAX ||
        root >= count || table < 8 || table > size - 32 || count > (size - 32 - table) / tail[6])
        return -1;
    p->data = data;
    p->size = size;
    p->table = (size_t)table;
    p->count = (uint32_t)count;
    p->root = (uint32_t)root;
    p->offset_size = tail[6];
    p->ref_size = tail[7];
    return 0;
}
...
static int object(const bplist_t *p, uint32_t ref, unsigned *type, size_t *start, size_t *count)
{
    ...
    *type = marker >> 4;
    ...
    else if (n == 15)
    {
        ...
        if (bytes > p->table - pos)
            return -1;
        n = read_be(p->data + pos, bytes);
        pos += bytes;
    }
    size_t width = (*type == 6)    ? 2
                   : (*type == 10) ? p->ref_size
                   : (*type == 13) ? 2 * p->ref_size
                                   : 1;
    if (n > (p->table - pos) / width)
        return -1;
    *start = (size_t)pos;
    *count = (size_t)n;
    return 0;
...
uint32_t bplist_get(const bplist_t *p, uint32_t dict, const char *key)
{
    unsigned type;
    size_t pos, n;
    if (!key || object(p, dict, &type, &pos, &n) || type != 13)
        return BPLIST_NONE;
...
uint32_t bplist_at(const bplist_t *p, uint32_t array, size_t index)
{
    unsigned type;
    size_t pos, n;
    if (object(p, array, &type, &pos, &n) || type != 10 || index >= n)
        return BPLIST_NONE;
...
int bplist_uint(const bplist_t *p, uint32_t ref, uint64_t *value)
{
    unsigned type;
    size_t pos, n;
    if (!value || object(p, ref, &type, &pos, &n) || type != 1)
        return -1;
...
int bplist_real(const bplist_t *p, uint32_t ref, double *value)
{
    unsigned type;
    size_t pos, n;
    if (!value || object(p, ref, &type, &pos, &n))
        return -1;
...
static void append(bplist_writer_t *w, uint64_t v, unsigned bytes)
{
    ...
    for (unsigned i = bytes; i; --i)
        w->data[w->used++] = (uint8_t)(v >> ((i - 1) * 8));
}
...
static uint32_t bytes(bplist_writer_t *w, unsigned type, const uint8_t *data, size_t n)
{
    ...
    if (w->failed || n > w->capacity - w->used || (n && !data))
    {
        w->failed = 1;
        return BPLIST_NONE;
    }
    if (n)
        memcpy(w->data + w->used, data, n);
    w->used += n;
    return ref;
}
...
uint32_t bplist_add_data(bplist_writer_t *w, const uint8_t *data, size_t n)
{
    return bytes(w, 4, data, n);
}
uint32_t bplist_add_uint(bplist_writer_t *w, uint64_t value)
{
    uint32_t ref = start(w, 1, 3);
    append(w, value, 8);
    return ref;
}
...
{
    if (root >= w->count || w->failed)
        return 0;
    size_t table = w->used;
    for (uint32_t i = 0; i < w->count; ++i)
        append(w, w->offsets[i], 4);
    append(w, 0, 6);
    append(w, 4, 1);
    append(w, 1, 1);
    append(w, w->count, 8);
