                        ...
                        uint8_t *ready, int timeout_ms, int writing)
{
    ...
    for (;;)
    {
        ...
        result = select(0, writing ? NULL : &fds,
        ...
        if (result > 0)
        {
            ...
            {
                if (socket_valid(&sockets[i]) &&
                    FD_ISSET((SOCKET)sockets[i].handle, &fds))
                {
                    ready[i] = 1;
                    total++;
                }
                 ...
                 net_addr_t *peer, int timeout_ms)
{
    ...
    if (result != 0)
        return result;
    result = (int)recvfrom((SOCKET)sock->handle, (char *)data, (int)capacity,
                 ...
                 const net_addr_t *peer)
{
    ...
    if (!socket_valid(sock) || (!data && length) || length > INT_MAX || !peer ||
        ...
        return NET_ERROR;
    result = (int)sendto((SOCKET)sock->handle, data ? (const char *)data : "",
