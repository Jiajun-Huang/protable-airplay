...
static int make_address(struct sockaddr_in *addr, const char *ip, uint16_t port)
{
    ...
    return inet_pton(AF_INET, ip, &addr->sin_addr) == 1 ? 0 : NET_ERROR;
}
...
static int copy_peer(net_addr_t *peer, const struct sockaddr_in *addr)
{
    if (!peer)
        return 0;
    if (!inet_ntop(AF_INET, &addr->sin_addr, peer->ip, sizeof(peer->ip)))
        return NET_ERROR;
...
static int open_bound(net_socket_t *sock, const char *ip, uint16_t port, int type)
{
    ...
    if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, (const char *)&reuse, sizeof(reuse)) != 0 ||
        ...
        goto fail;
    if (type == SOCK_STREAM &&
        setsockopt(fd, SOL_SOCKET, SO_NOSIGPIPE, (const char *)&reuse, sizeof(reuse)) != 0)
        goto fail;
...
int net_tcp_accept(net_socket_t *listener, net_socket_t *client, net_addr_t *peer, int timeout_ms)
{
    ...
    if (result != 0)
        return result;
    fd = accept((int)listener->handle, (struct sockaddr *)&addr, &length);
    if (fd == -1)
    ...
    {
        int enabled = 1;
        if (setsockopt(fd, SOL_SOCKET, SO_NOSIGPIPE, (const char *)&enabled, sizeof(enabled)) != 0)
        {
            close(fd);
            return NET_ERROR;
        }
...
int net_tcp_recv(net_socket_t *sock, void *data, size_t capacity, int timeout_ms)
{
    ...
    if (result != 0)
        return result;
    result = (int)recv((int)sock->handle, (char *)data, (int)capacity, 0);
    if (result < 0)
        return would_block(errno) ? NET_TIMEOUT : NET_ERROR;
    return result;
}
...
int net_tcp_send_all(net_socket_t *sock, const void *data, size_t length, int timeout_ms)
{
    ...
    while (sent < length)
    {
        ...
        if (chunk > INT_MAX)
            chunk = INT_MAX;
        result = (int)send((int)sock->handle, bytes + sent, (int)chunk, 0);
        if (result > 0)
            sent += (size_t)result;
        else if (result == 0)
            return NET_ERROR;
...
int net_udp_recv(net_socket_t *sock, void *data, size_t capacity, net_addr_t *peer, int timeout_ms)
{
        ...
        return result;
    {
        ...
        result = (int)recvmsg((int)sock->handle, &message, 0);
