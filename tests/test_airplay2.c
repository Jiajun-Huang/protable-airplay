...
static void test_plist(void)
{
    ...
    uint8_t data[256];
    size_t size = unhex("62706c6973743030d20102030454726174655773747265616d73233ff0000000000000a105"
                        ...
                        "0000000a00000000000000000000000000000058",
                        data);
    bplist_t p;
    double rate;
    uint64_t type;
...
static void test_records(void)
{
    ...
    size_t size = unhex(
        "160057e81678e2a8f5f139410e35fc136116d681feffeca7cfb090faa6fb8d99ace065591b18a2cd", record);
    static const char text[] = "OPTIONS * RTSP/1.0\r\n\r\n";
    pairing_t p = {0};
    p.established = 1;
    for (unsigned i = 0; i < 32; ++i)
        p.read_key[i] = p.write_key[i] = (uint8_t)i;
...
static void test_ptp(void)
{
    ...
    uint8_t follow[44];
    memcpy(follow, sync, sizeof(follow));
    follow[0] = 0x18;
    follow[6] = 0;
    ...
    airplay_anchor_t anchor = {.clock_id = 42,
                               .network_us = 3000000,
                               .rtp_time = UINT32_MAX - 44099,
                               .valid = 1,
                               .playing = 1};
    uint64_t deadline;
...
static void test_fairplay(void)
{
    ...
    for (unsigned i = 12; i < sizeof(second); ++i)
        second[i] = (uint8_t)i;
