...
static void check_decode(const uint8_t *data, size_t length, const int16_t *expected, unsigned channels)
{
    ...
    CHECK(alac_decoder_decode_frame(&decoder, invalid, sizeof(invalid), output, &count, 704) < 0 && count == 0);
    if (channels == 2)
        CHECK(alac_decoder_decode_frame(&decoder, mono_packet, sizeof(mono_packet), output, &count, 704) < 0 && count == 0);
    for (size_t short_length = 1; short_length + 3 < length; ++short_length)
        CHECK(alac_decoder_decode_frame(&decoder, data, short_length, output, &count, 704) < 0 && count == 0);
...
static void check_encrypted_packet(void)
{
    ...
    CHECK(mbedtls_aes_crypt_cbc(&aes, MBEDTLS_AES_ENCRYPT, encrypted_bytes, working_iv, tone_packet, encrypted) == 0);
    memcpy(encrypted + encrypted_bytes, tone_packet + encrypted_bytes, sizeof(tone_packet) - encrypted_bytes);
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        memcpy(decrypted + encrypted_bytes, encrypted + encrypted_bytes, sizeof(tone_packet) - encrypted_bytes);
...
int main(void)
{
    ...
    const uint8_t raop_silence[] = {
        0x20,0x00,0x00,0x04,0x00,0x13,0x08,0x09,0x81,0xf8,0xc1,0xff,0x80,0x00,0x00,0x13,
        0x08,0x09,0x81,0xf8,0xc1,0xff,0x80,0x00,0x00,0xff,0x80,0xaf,0xbf,0xe0,0x2b,0xfc
    };
